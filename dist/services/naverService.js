import { createStealthPage, testProxy } from "../utils/browser.js";
// Human-like random delays with variation
const humanDelay = (min = 1000, max = 3000) => new Promise(resolve => setTimeout(resolve, Math.random() * (max - min) + min));
// Random browsing pattern
async function simulateBrowsingBehavior(page) {
    ...
    for (let i = 0; i < scrollSteps; i++) {
        ...
        if (Math.random() > 0.7) {
            ...
            if (viewport) {
                await page.mouse.move(Math.random() * viewport.width, Math.random() * viewport.height, { steps: Math.floor(Math.random() * 8) + 3 });
            }
        }
    }
...
export async function scrapeNaver(query, pageIndex = 1) {
