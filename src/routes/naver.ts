...
console.log("🔄 Naver route initialized");
...
router.get("/", async (req, res) => {
  console.log("🔄 Naver route GET /api/naver?q=query&page=1 called");
  try {
    const query = req.query.q as string || "아이폰";
    const page = parseInt(req.query.page as string || "1");
    const method = req.query.method as string || "browser";
    ...
    console.log("🔄 Naver route scrapeNaver result:", result);
    if (result.success) {
      console.log("🔄 Naver route result success");
      res.json({
        success: true,
        query,
        page,
      ...
      console.log("🔄 Naver route result error");
      res.status(500).json({
        success: false,
        error: result.error,
        executionTime: result.executionTime,
      ...
      console.error("❌ Naver route error:", err);
    res.status(500).json({  console.log("🔄 Naver route error:", err),
      success: false,
...
router.get("/health", async (req, res) => {
  console.log("🔄 Naver route health check called");
    res.json({ 
    status: "OK", 
